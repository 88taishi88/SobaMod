...
public class ItemBuckwheatSeed extends ItemSeeds implements IPlantable {

	public ItemBuckwheatSeed() {
		super(RegisterBlock.Buckwheat, Blocks.farmland);
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (side != 1) {
			...
			if (world.getBlock(x, y, z).canSustainPlant(world, x, y, z, ForgeDirection.UP, this) && world.isAirBlock(x, y + 1, z)) {
				// 作物を設置する。
				world.setBlock(x, y + 1, z, RegisterBlock.Buckwheat);
				// スタック数を減らす。
				--itemStack.stackSize;
				return true;
			} else {
	...
	public Block getPlant(IBlockAccess world, int x, int y, int z) {
		// IPlantableの実装。
		return RegisterBlock.Buckwheat;
	}
